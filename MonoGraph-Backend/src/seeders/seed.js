  ...
  try {
    ...
    for (let i = 0; i < shopsData.length; i++) {
      ...
      if (newShop?._id) {
        ...
        if (shopName) {
          shopNameToNewShop.set(normalize(shopName), newShop);
        }
      }
    }

    console.log(`Built ${shopNameToNewShop.size} shop name mappings`);
    ...
    for (const item of itemsData) {
      ...
      if (!shop) {
        throw new Error(`Shop "${shopName}" not found for item "${itemName}"`);
      }

      if (!shop.category) {
        throw new Error(`Shop "${shopName}" does not have a category`);
      }
