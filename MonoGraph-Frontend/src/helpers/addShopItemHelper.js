...
export const buildDefaultWorkingHours = () =>
  WEEK_DAYS.map((day) => ({ day, open: '09:00', close: '18:00', isClosed: false }));
...
const translationFromForm = (form) =>
  buildTranslation(
    form.titleEn.trim(),
    form.titleFa.trim(),
    form.titlePs.trim(),
...
export const buildBusinessPayload = async ({ form, coverFile, profileFile }) => {
  ...
  if (profileFile)
    payload.append('profile', await toFormDataFile(profileFile, 'business-profile.jpg'));
  return payload;
};
...
export const businessFormFromShop = (shop = {}) => ({
  titleEn: shop.translation?.en?.title || '',
  titleFa: shop.translation?.fa?.title || '',
  titlePs: shop.translation?.ps?.title || '',
  description: shop.translation?.en?.description || '',
  categoryId: shop.category?._id || shop.category || '',
  city: shop.city || 'herat',
  phones: shop.phone?.length ? shop.phone : [''],
  email: shop.email || '',
  workingHours: shop.workingHours?.length ? shop.workingHours : buildDefaultWorkingHours(),
  socialLinks: Object.entries(shop.social || {}).map(([platform, url]) => ({ platform, url })),
});

export const itemFormFromItem = (item = {}) => ({
  titleEn: item.translation?.en?.title || '',
  titleFa: item.translation?.fa?.title || '',
  titlePs: item.translation?.ps?.title || '',
  description: item.translation?.en?.description || '',
  price: item.price == null ? '' : String(item.price),
  businessId: item.shop?._id || item.shop || '',
  city: item.city || 'herat',
